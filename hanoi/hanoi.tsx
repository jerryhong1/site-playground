...
const validNumRings = (nRings: number | undefined) => {
  return nRings && nRings <= 8 && nRings >= 2
}

const validNumTrials = (ntTrials: number | undefined) => {
  return ntTrials && ntTrials <= 1000000 && ntTrials > 20
}

// helper functions

const sampleFromList = (A: any[]) => {
  return A[Math.random() * A.length >> 0]
}

const sampleVariance = (A: number[]) => {
  const sampleMean = average(A)
  const n = A.length
  let sum = 0
  for (let i = 0; i < n; i++) {
    sum += (A[i] - sampleMean) ** 2
  }
  return sum / (n - 1.0)
}
...
const getSamples = (hanoi: Hanoi, numTrials: number): number[] => {
